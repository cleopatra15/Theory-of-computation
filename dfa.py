...
def get_section_data(sections_dict, section_name):
    ...
    # Convert to lower to handle '[Stari]' or '[stari]' safely
...
def parse_transitions(transition_lines):
    ...
    for line in transition_lines:
        # Clean the string: remove parentheses
        clean_line = line.replace('(', '').replace(')', '')
        
        # Split "(q0,0)-q0" -> "q0,0" and "q0"
...
def print_transition_table(rules, states, alphabet):
    ...
    # Create the header row (e.g., State | 0 | 1 )
    # We use :<10 to pad the strings with spaces so the columns align perfectly
    ...
    for state in sorted(states):
        ...
        for sym in alphabet:
            ...
            if state in rules and sym in rules[state]:
                # Join the destination states. 
                # (Using a comma makes it look great for NFAs later: q0,q1)
                destinations = ",".join(rules[state][sym])
                row_string += f"{destinations:<10} | "
            else:
                # Print a dash if there is no valid transition (Dead end)
                row_string += f"{'-':<10} | "
...
def simulate_dfa(rules, initial_state, final_states, input_string):
    ...
    for symbol in input_string:
        # Check if the state exists in rules AND has a transition for this symbol
        if current_state in rules and symbol in rules[current_state]:
            # For a DFA, take the first destination [0]
            current_state = rules[current_state][symbol][0]
        else:
            # Dead end, no transition exists
